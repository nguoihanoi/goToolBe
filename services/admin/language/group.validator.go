...
func ValidateCreateGroupInput(ctx *fastHttp.RequestCtx) (regRequest CreateGroupInput, status bool) {
	...
	libProcess.Try(func() {
		...
		if otherGroupDetail.ID != "" {
			libUtilities.Response().SendError(ctx, "The group code has already been used.", nil, 206)
		}
		status = true
...
func ValidateUpdateGroupInput(ctx *fastHttp.RequestCtx) (regRequest UpdateGroupInput, status bool) {
	...
	libProcess.Try(func() {
		...
		if groupDetail.ID == "" {
			libUtilities.Response().SendError(ctx, "This group code information does not exist in the system.", nil, 206)
		}
		status = true
	}).Catch(func(e libProcess.E) {
		libUtilities.Response().SendError(ctx, "Invalid input data!", e, 206)
