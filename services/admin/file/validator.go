...
func ValidateCreateInput(ctx *fastHttp.RequestCtx) (regRequest CreateInput, status bool) {
	...
	libProcess.Try(func() {
		...
		wg.Wait()
		if userDetail.ID == "" {
			libUtilities.Response().SendError(ctx, "You do not have file to perform this function.", nil, 206)
			return
		}
...
func ValidateUpdateInput(ctx *fastHttp.RequestCtx) (regRequest UpdateInput, status bool) {
	...
	libProcess.Try(func() {
		...
		go func() {
			defer wg.Done()
			fileDetail = fileModel.GetById(regRequest.Id, true)
		}()
		go func() {
			defer wg.Done()
			langCode := languageModel.GetCodes()
		...
		if fileDetail.ID == "" {
			libUtilities.Response().SendError(ctx, "This file information does not exist in the system.", nil, 206)
			return
		}
		switch statusValidate {
		case 1, 2:
...
func ValidateSearchFileInput(ctx *fastHttp.RequestCtx) (regRequest SearchFileInput, status bool) {
	status = false
	libProcess.Try(func() {
		//Todo: get struct input
		err := libUtilities.Validate(ctx, &regRequest)
