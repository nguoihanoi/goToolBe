...
func ValidateCreateInput(ctx *fastHttp.RequestCtx) (regRequest CreateInput, status bool) {
	...
	libProcess.Try(func() {
		...
		wg.Wait()
		if userDetail.ID == "" {
			response.SendError(ctx, "You do not have file to perform this function.", nil, 206)
			return
		}
...
func ValidateUpdateInput(ctx *fastHttp.RequestCtx) (regRequest UpdateInput, status bool) {
	...
	libProcess.Try(func() {
		...
		if fileDetail.ID == "" {
			response.SendError(ctx, "This file information does not exist in the system.", nil, 206)
			return
		}
		switch statusValidate {
		case 1, 2:
